### redis服务搭建
> 在脚本调度中, 需要有个中间件共享一些, token数据等, 选用redis进行数据共享, 前期有点大材小用, 后期扩展其他功能直接用也方便一些
* [redis镜像](https://hub.docker.com/_/redis/tags)
* [官方文档](https://redis.io/docs/getting-started/installation/install-redis-on-linux/)

#### redis 部署
##### 创建空间
```sh
$ kubectl create namespace redis-space
```
...
* 安装命令
```yaml
$ helm install redis -f redis-helm.yaml my-repo/redis  --namespace redis-space
```

#### redis 管理客户端
> 电脑内存太小, 知道的东西太多, 命令真记不住, 提高使用效率, 转薅服务器羊毛

##### 工具选型
* github上选取 1k star 以上的工具, 优先考虑简单, 监控, 操作功能

名称|stars|体验网址|功能|看法|有无docker版
:-|:-|:-|:-|:-|:-
[RedisLive](https://github.com/nkrode/RedisLive)|3.1k|[已过期](http://www.nkrode.com/article/real-time-dashboard-for-redis)|监控|功能过少, 需要操作内部数据|&#10006;
[redis-manager](https://github.com/ngbdf/redis-manager)|2.8k|-|监控、安装、管理、告警、命令行操作|功能过于庞大, 很多东西用不上繁重|&#10003;
[phpRedisAdmin](https://github.com/ErikDubbelboer/phpRedisAdmin)|3.0k|[跳转体验](https://dubbelboer.com/phpRedisAdmin/?overview)|操作|界面过于简陋, 不带监控|&#10003;|
[redmon](https://github.com/steelThread/redmon)|1.6k|-|监控、命令行操作|6-7年没更新、操作体验差|&#10006;
[webdis](https://github.com/nicolasff/webdis)|2.6k|-|api连接redis的代理|没有可视化界面, 操作麻烦|&#10006;
[redis-admin](https://github.com/mauersu/redis-admin)|453|-|操作|没有监控|&#10006;

* 没有完全想要的, 暂时选定 phpRedisAdmin, 操作和 redis dekstop manage 也差不多, 有docker版本, 也有现成的镜像, 可直接部署到 K8S上 省事儿, 但是好像是 php 的限制性, 使用重定向方式导致功能不全

* 无法解决, 用了两种方式去制作都无法解决
![](./images/2023-02-17-15-03-12.png)

* 查issues 找到错误原因, 但是镜像没改过来, 只能自己重新打包镜像
![](./images/2023-02-17-15-50-03.png)
...
# 打包镜像
$ docker build -t fons/phpredisadmin:latest .

# 打标签
$ docker tag fons/phpredisadmin:latest 10.108.6.48:5000/fons/phpredisadmin:latest

# push 私服
$ docker push 10.108.6.48:5000/fons/phpredisadmin:latest
```

#### phpRedisAdmin 部署
* 看了一下 `dockerfile` 文件, 他把 `config.environment.inc.php` 复制到容器里面 `config.inc.php`, 挺意义不明的, 起初以为它改了这个配置读取方式, 先装了 `pod` 进容器看了一下, 并没有, 获取配置方式还是没变, 读的 `config.sample.inc.php` 文件

* 脚本内优先选用 `config.inc.php`, 所以需要用 `config.sample.inc.php` 覆盖掉 `config.inc.php`
```php
...
apiVersion: v1
metadata:
  name: redis-admin-configs
  namespace: redis-space
  labels:
    app.kubernetes.io/name: redis-admin
...
spec:
  ...
  template:
    ...
    spec:
      ...
      - name: redis-admin
        # 选择私服
        image: 10.108.6.48:5000/fons/phpredisadmin
        ports:
          - containerPort: 80
            name: web-port
            protocol: TCP
        volumeMounts:
        - name: redis-admin-config
          mountPath: /src/app/includes/config.inc.php #容器内的挂载点
          subPath: common_properties
      volumes:
      - name: redis-admin-config            #指定名称必须与上面一致
        configMap:
          name: redis-admin-configs      # 和configmap 名称一样即可
...
spec:
  ...
  - name: redis-admin
    ...
    env:
      - name: REDIS_1_HOST
        value: "redis-headless.redis-space.svc.cluster.local"
      - name: REDIS_1_NAME
        value: "default_redis"
      - name: REDIS_1_PORT
        value: "6379"
...
##### 端口映射
```sh
$ kubectl port-forward --address 0.0.0.0 --namespace redis-space service/redis-admin-service 8080:80
```