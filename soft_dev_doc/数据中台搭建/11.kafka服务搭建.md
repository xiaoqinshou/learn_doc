### kafka服务搭建
> 在离线实时计算中, 需要有个中间件进行数据的流化处理, 前期主要是离线计算不点大材小用, 但后期扩展实时计算其他功能直接用也方便一些
...
##### 创建空间
```sh
$ kubectl create namespace kafka-space
```
...
##
## kafka zookeeper 所在路径
zookeeperChrootPath: "/queue/kafka"
...
externalZookeeper:
  ...
  ##
  servers: ["zookeeper-headless.zookeeper-space.svc.cluster.local:2181"]

```

* 安装命令
```yaml
$ helm install kafka -f kafka-helm.yaml my-repo/kafka  --namespace kafka-space
```

#### kafka 管理客户端
> 知道的东西太多, 命令真记不住, 提高工作效率, 寻找好用的可视化管理端


##### 工具选型
* github上选取 1k star 以上的工具, 优先考虑简单, 显示, 操作功能


名称|stars|体验网址|功能|看法|有无docker版
:-|:-|:-|:-|:-|:-
[kafdrop](https://github.com/obsidiandynamics/kafdrop)|4.3k|-|显示信息(brokes, topics, patitions, consumers and message)|基础的查询数据均有、但无操作功能、能|&#10003;
[kafka-ui](https://github.com/provectus/kafka-ui)|5k|-|监控、管理、操作、权限、数据屏蔽|功能强大, 英文界面|&#10003;
[kafka-map](https://github.com/dushixiang/kafka-map)|446|-|显示信息和操作|几乎想要的功能都有了就是star数少了一点|&#10003;|

* 选定 kafka-ui, 虽然没有中文界面, 但功能却是强大, UI操作感也会更舒适一些
...
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/author: provectuslabs
spec:
  replicas: 1
  selector:
    matchLabels:
      ...
      - name: kafka-ui
        ...
        env:
          - name: KAFKA_CLUSTERS_0_BOOTSTRAPSERVERS
            value: "kafka-headless.kafka-space.svc.cluster.local:9092"
...
##### 端口映射
```sh
$ kubectl port-forward --address 0.0.0.0 --namespace kafka-space service/kafka-ui-service 8080:8080
```